...
public class Raffle {
  ...
  /**
   * Вручную задайте вес проигрыша. Относительные вероятности в розыгрыше
   * пересчитаются соответственно.
   *
  ...
  /**
   * Здесь собраны методы, отвечающие за учет остатков призов.Чтобы не плодить
   * сущности,
   * заводя новый ChanceCalc на каждый выигрыш, является внутренним классом, и
   * использует объект cc из Raffle
...
class ChanceCalc {
  ...
  /**
   * Выдает случайное число от 0 до maxChance. Здесь можно корректировать диапазон
   * при изменении механики весов.
   *
  ...
  /**
   * первоначальная обработка списка игрушек для выставления шансов, формула
   * нормализации весов:
  ...
  /**
   * метод, чтобы по настоящему случайно выбрать игрушку с одинаковыми весами
   * вероятности, иначе очередь всегда будет брать
   * левый нижестоящий узел(который постоянен из-за упорядочения по другим
   * значениям объекта, помимо значения в компараторе очереди)
   */
